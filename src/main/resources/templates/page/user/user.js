				...
				function() {
					...
					layui.upload({
						...
						success : function(res) {
							...
							window.sessionStorage.setItem('userFace',
									res.data[num].src);
						}
					});
					...
					if (window.sessionStorage.getItem('userInfo')) {
						var userInfo = JSON.parse(window.sessionStorage
								.getItem('userInfo'));
						var citys;
						$(".realName").val(userInfo.realName); // 用户名
						$(".userSex input[value=" + userInfo.sex + "]").attr(
								"checked", "checked"); // 性别
						$(".userPhone").val(userInfo.userPhone); // 手机号
						$(".userBirthday").val(userInfo.userBirthday); // 出生年月
						$(".userAddress select[name='province']").val(
								userInfo.province); // 省
						// 填充省份信息，同时调取市级信息列表
						...
						if (count > 0) {
							...
							$form.find('select[name=area]').attr("disabled",
									"disabled");
						}
						$(".userAddress select[name='area']")
								.val(userInfo.area); // 区
						for (key in userInfo) {
							if (key.indexOf("like") != -1) {
								$(".userHobby input[name='" + key + "']").attr(
										"checked", "checked");
							}
						}
					...
					form.on("submit(changePwd)", function(data) {
						...
						$.ajax(
								"http://127.0.0.1:1011/tsyUserApi/modifyUserPwd/"
										+ pwd, {
									type : "PUT",
									dataType : "json",
									timeout : 6000,
...
function loadProvince() {
	...
	for (var i = 0; i < areaData.length; i++) {
		proHtml += '<option value="' + areaData[i].provinceCode + '_'
				+ areaData[i].mallCityList.length + '_' + i + '">'
...
function loadCity(citys) {
	...
	for (var i = 0; i < citys.length; i++) {
		cityHtml += '<option value="' + citys[i].cityCode + '_'
				+ citys[i].mallAreaList.length + '_' + i + '">'
...
function loadArea(areas) {
    ...
    form.render();
    form.on('select(area)', function(data) {});
>>>>>>> branch 'master' of https://github.com/yanfayibu/yphting-backstage.git
}